	...
	public void write()
	{
		try
		{
			...
			for(Entry<String, Data> l:entry)
			{
				...
				e = doc.createElement("Tie");
				e.appendChild(doc.createTextNode(df.format(d.getTiePercentage())));
				pocket.appendChild(e);

				e = doc.createElement("Loss");
				e.appendChild(doc.createTextNode(df.format(d.getLossPercentage())));
				pocket.appendChild(e);

				e = doc.createElement("nbOpponantWinners");
				e.appendChild(doc.createTextNode(df.format(d.getAverageOpponantWinner())));
				pocket.appendChild(e);
